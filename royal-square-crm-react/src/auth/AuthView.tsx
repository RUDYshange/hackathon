...
/**
 * Sovereign Wealth Modern Sign-in & Sign-up screen, backed by the real Django
 * auth API. It validates input, calls register/login, and hands the resulting
 * session (token + account) to <App/>, which routes to the matching workspace.
 *
        ...
        <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200/90 p-7 md:p-8 shadow-sm">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4" noValidate>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {submitting
              ...
              <div className="space-y-2">
                {DEMO_ACCOUNTS.map((a) => (
                  <button
                    ...
                    onClick={() => useDemo(a)}
                    disabled={submitting}
                    className="group flex w-full items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left transition hover:border-blue-300 hover:shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
                  >
