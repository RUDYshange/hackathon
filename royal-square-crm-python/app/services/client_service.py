...
class ClientService:
    ...
    def compute_balance_sheet(client: Client) -> Dict[str, Any]:
        ...
        for e in client.ledger_entries:
            line = {
                "id": e.id,
                "label": e.label,
                "amount": e.amount,
    ...
    def calculate_compliance_gaps(client: Client) -> int:
        present_types = {d.type for d in client.documents}
    ...
    @staticmethod
    def list_clients(db: Session, search: Optional[str] = None) -> List[Dict[str, Any]]:
        clients = ClientRepository.get_all(db, search)
        return [ClientService.to_summary(c) for c in clients]

    @staticmethod
    def get_client_detail(db: Session, client_id: str) -> Optional[Dict[str, Any]]:
    ...
    @staticmethod
    def create_client(db: Session, data: dict) -> Dict[str, Any]:
        ...
        client = ClientRepository.create(db, {k: v for k, v in mapped.items() if v is not None})
        return ClientService.to_detail(client)
